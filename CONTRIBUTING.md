...
本仓库为课题组维护的开源仓库（MIT 协议）。**`main` 不接受任何直接推送**——所有改动一律经分支 + Pull Request，由维护者审查后合并。以下流程对所有人生效；外部贡献者请先读「协作方式」。

## 协作方式（贡献者必读）

**仓库已公开，贡献者不再持有写权限**，无法直接推送分支或改动 `main`。请按 fork 流程贡献：
...
`main` 禁止直接推送（贡献者无写权限，见「协作方式」）。每个改动开一条短生命周期分支，用完即删：
...
> **关于分支保护**：本仓库是 Free 套餐的仓库，GitHub 不允许启用分支保护或
> ruleset（该功能要求 Pro 套餐，API 返回 `403 Upgrade to GitHub Pro`）。
>
> 因此本项目用**权限**代替服务端保护：仓库公开、贡献者不持有写权限，从机制上无法
